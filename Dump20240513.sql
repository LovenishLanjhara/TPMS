...
INSERT INTO `user` VALUES (1,'Manoj Singh','manoj','$2a$12$77eAVtQguUU6p0OWangJXOyx/yt6W1oWbamyi1zjZt1A8QobMfRm6',1,'9987666112','manojsingh@gmail.com',_binary '\0',1,'2024-03-22 11:56:13',NULL,'2024-04-19 15:34:48',_binary '\0'),(2,'Kiran','kiran@123','$2a$10$7UZJR0n/Tcpn7YhrUhdq/.1youX17/zm13jbXq.j5iTJdtNmcrE2u',2,'9987666113','kiran@gmail.com',_binary '',NULL,'2024-03-25 12:45:02',NULL,'2024-03-25 12:48:17',_binary ''),(3,'Bilu Barber','bilei','$2a$10$TGVzPGXjlTx9sfvhULEibeEK5cPpSlvz89J7l5HzGwWNoGG0E25RO',3,'2323221323','chidananda.sutar@csm.co.in',_binary '',1,'2024-04-24 17:00:23',NULL,NULL,_binary '\0'),(4,'satyam Dixit','satyam','$2a$10$KrFjTFiVk3Y2KAdQ/eeq1ODjGJuLJ2MGeqMvq23u/C64zSbT42MKK',1,'9616714339','satyam@gmail.com',_binary '',1,'2024-05-08 13:12:29',NULL,'2024-05-08 13:17:50',_binary '');
/*!40000 ALTER TABLE `user` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Dumping events for database 'tpms'
--

--
-- Dumping routines for database 'tpms'
--
/*!50003 DROP PROCEDURE IF EXISTS `TPMS_ATTENDANCE` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `TPMS_ATTENDANCE`(in varaction varchar(30), in varPlatformName varchar(20),in varDate varchar(20),in inYear varchar(10),in inputMonth varchar(10),in resNames varchar(200),in intActivityId int)
BEGIN
 
    if (varaction='ACTIVITY_ATTENDANCE1_ACTIVITY') then
...
-- where  allocation.activityDate=varDate 
        where  varDate BETWEEN allocation.activityFromDate AND allocation.activityToDate
        and allocation.activityAllocateId=intActivityId 
        and allocation.deletedFlag=0 and allDetails.deletedFlag=0
        and res.deletedFlag=0 
order by res.resourceName, allocation.activityFor;
    end if;
    
    if (varaction='ACTIVITY_ATTENDANCE2_ACTIVITY') then
select res.resourceId,res.resourceName,allDetails.activityAllocateDetId,
allocation.activityFor  ,
allocation.fromHours ,
allocation.toHours ,
        activity.activityName,
         ...
         allDetails.activityAllocateDetId,
          res.designation,
         res.resourceCode,
         IFNULL( atte.isPresent, 0) as isPresent
          -- 'S' as updateChk,
from activity_allocation allocation
inner join activity_allocation_details  allDetails  on allocation.activityAllocateId=allDetails.activityAllocateId
inner join activity activity on activity.activityId=allocation.activityId
inner join  resource_pool res on res.resourceId=allDetails.resourceId
left join attendance atte on  atte.activityAllocateDetId=allDetails.activityAllocateDetId
-- where allocation.activityDate=varDate 
        -- where  varDate BETWEEN allocation.activityFromDate AND allocation.activityToDate
        where  atte.atendanceDate=varDate
        and allocation.activityAllocateId=intActivityId and allocation.deletedFlag=0 and allDetails.deletedFlag=0
        -- and activity.activityId=intActivityId and allocation.deletedFlag=0 and allDetails.deletedFlag=0
        -- and activity.deletedFlag=0 
        and res.deletedFlag=0
order by res.resourceName, allocation.activityFor;
    end if;
    
    if (varaction='ATTENDANCE_REPORT_DATA') then
SELECT att.isPresent,CASE
        WHEN att.isPresent = 1 THEN 'P'
        WHEN att.isPresent = 0 THEN 'A'
        ELSE 'Unknown'
    END AS attendanceStatus,
    DATE_FORMAT(att.atendanceDate, '%d %b %Y') as atendanceDate,
   -- att.atendanceDate,
...
WHERE att.atendanceDate >= varPlatformName -- varPlatformName used as  fromdate
AND att.atendanceDate <= varDate    -- varPlatformName used as  toDate
     AND (
    (resNames = '0' OR CONCAT(res.resourceName, '(', res.resourceCode, ')') = resNames))
    AND (inYear = 0 OR  find_in_set ( alo.activityId,inYear)) 
 --   AND (inYear = 0 OR alo.activityId = inYear)  
     order by att.atendanceDate,res.resourceName,att.atendanceFor;
    end if;
    
    if (varaction='ASSESMENT_REPORT_DATA') then
    select ass.*,res.resourceName,res.resourceCode,res.platform,act.activityName,res.designation from assessment ass
    inner join resource_pool res on res.resourceId=ass.resourceId
    inner join activity act on act.activityId=ass.activityId
	WHERE ass.asesmentDate >= varPlatformName -- varPlatformName used as  fromdate
			AND ass.asesmentDate <= varDate    -- varPlatformName used as  toDate
            AND (
			(resNames = '0' OR CONCAT(res.resourceName, '(', res.resourceCode, ')') = resNames))
            and (inYear = 0 OR find_in_set ( ass.activityId,inYear))
             order by ass.asesmentDate;
    end if;
...
-- Dump completed on 2024-05-13 15:22:10
